    ...
    checkout: async(req,res,next)=>{
        ...
        try {
            ...
            if(errorBookIdStock.length!=0){
                return res.status(400).json({message: `book stock is not enough with id : ${errorBookIdStock.join(', ')} and user : ${user}`});
            }

            if(errorBookIdNotFound.length!=0){
                return res.status(400).json({message:`no books with id : ${errorBookIdNotFound.join(', ')} and user :${user}`})
            }
